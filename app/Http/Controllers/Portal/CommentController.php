...
class CommentController extends Controller
{
    public function all(Request $request)
    {
        if ($request->isMethod('post')){
            if ($request->_type == 'data' && $request->_data == 'all'){
                ...
                foreach (Comment::with('post')
                    ...
                    $data[] = [
                        ...
                        '<div class="btn-group">
                            <a class="btn btn-outline-primary bt-sm btn-view" href="'.route('portal.comment.detail', $comment->comment_id).'"><i class="icon-eye"></i></a>
            ...
            return view('portal.comment_all');
        }
    }
    ...
    public function detail(Request $request, $id)
    {
        if ($request->isMethod('post')){
            ...
            $parent = Comment::parent($id)->count() < 1 ? null : Comment::parent($id)->get();
            $data = ['comment' => $comment[0], 'parent' => $parent];
            return view('portal.comment_detail', $data);
        }
    }
    ...
    public function test()
    {
        $comment = Comment::with('post')
            ->where('comment_id', 1)
            ->where('comment_parent', null)->get();
        return response()->json($comment[0]->comment_id);
    }
}
